...
var attrs = {
  ...
  event: function event (key, val, attrs, tag) {
    ...
    var method = eventType.replace(':', '').split('-').map(function (v) { return capitalize(v); }).join('');
    attrs[wxmlEventName] = "handle" + method + "Proxy";
    console.log('eventName', eventName, eventMap);
